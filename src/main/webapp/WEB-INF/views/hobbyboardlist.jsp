...
<body>
	<%@ include file="/include/hobbyheader.jsp"%>
	<hr />
	<table align="center" width="90%" border="1" style="margin: 1%">
		...
		<!-- 게시물 생성 -> 리스트화 -->
		<c:if test="${lu.totalCount>0}">
			<c:forEach items="${lu.hlist}" var="hvo" varStatus="index">
				<tr>
					<td align="center">${lu.totalCount-(lu.currentPage-1)*lu.pageSize - index.count+1}</td>
					<td><a
						href="hobbyboard.do?totalCount=${lu.totalCount}&page=${lu.currentPage}&idx=${hvo.con_no}&category=${category.category_no}&log=${log}">${hvo.title}</a></td>
					<td align="center">${hvo.update_at}</td>
					<td align="center">${hvo.m_name}</td>
				</tr>
			</c:forEach>
		</c:if>
		...
		<tr>
			<td align="right" colspan="4"><input type="button"
				onclick="location.href='hobbyboardwrite.do?log=${log}&category=${category.category_no}'"
				value="글쓰기"></td>
		</tr>
	</table>
</body>
