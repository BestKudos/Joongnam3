...
import mybatis.service.SelectInformationListService;
 ...
 */
@Controller
public class InformationBoardListController {
	
	private static final Logger logger = LoggerFactory.getLogger(InformationBoardListController.class);

	@Autowired
	SelectInformationListService selectListService;
	@Autowired
	 ...
	 * @throws IOException 
	 */
	@RequestMapping("informationboardlist.do")
	public ModelAndView informationboardlist(Locale locale,HttpServletRequest request,HttpServletResponse response) throws IOException {
		logger.info("Welcome InformationBoardList! The client locale is {}.", locale);
		...
		modelAndView.setViewName("informationboardlist");
		
		if(idx != 0 && (confirm != 2 && confirm !=3))
			response.sendRedirect("informationboard");
